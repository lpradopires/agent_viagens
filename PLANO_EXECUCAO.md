...
- **Ações:**
  ...
  - [x] **Professor `luizcs-araujo` com acesso aos dois artefatos:** convite de colaborador no repositório com permissão `write` (aguardando aceite dele — o GitHub sempre exige) e adicionado como `READER` no GitHub Project 10, já que o quadro é privado e o PDF exige que o professor acesse os artefatos de avaliação
- **Definição de Pronto:** ✅ `main` contém a versão final e funcional; Kanban refletindo o estado real; professor com acesso ao repositório e ao quadro.
