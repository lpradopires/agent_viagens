...
- **Mecanismo de Fallback Automático:** Adicionado um bloco de tratamento de exceção em `agentNode` no [src/agent.ts](file:///Users/leandropradopires/Projetos/mini_projeto/src/agent.ts). Se o modelo principal falhar devido a erros de limite de cota/tokens (429/413) da conta free, o fluxo aciona um fallback automático e transparente usando o OpenRouter, impedindo travamento de execução e mantendo o agente operacional de forma ininterrupta.
- **Suporte ao Modelo GPT-4.1 Nano:** Adicionada e priorizada a inicialização do modelo `"openai/gpt-4.1-nano"` do OpenAI via OpenRouter como o modelo secundário principal (após o Gemini) quando a chave `OPENROUTER_API_KEY` estiver ativa. Esse modelo oferece processamento rápido e alta fidelidade em tool-use.
- **Limite de Recursão e Segurança de Grafo:** Configurado um limite estrito de recursão (`recursionLimit: 15`) na chamada de execução do grafo LangGraph no arquivo [src/index.ts](file:///Users/leandropradopires/Projetos/mini_projeto/src/index.ts). Se o agente entrar em um loop de transição que exceda 15 passos, o LangGraph encerra a execução com `GraphRecursionError`, o qual é tratado para alertar o usuário sem consumir tokens adicionais.
- **Mecanismo de Seleção de Provedores:** Adicionado o parâmetro `TRAVEL_API_PROVIDER` no arquivo [`.env`](file:///Users/leandropradopires/Projetos/mini_projeto/.env) que permite alternar as APIs de busca entre `GeckoAPI` e `Duffel` dinamicamente.
- **Integração com a API da Duffel:** Implementados o cliente [src/duffel_api_client.ts](file:///Users/leandropradopires/Projetos/mini_projeto/src/duffel_api_client.ts) e as ferramentas MCP [src/duffel_tools.ts](file:///Users/leandropradopires/Projetos/mini_projeto/src/duffel_tools.ts) para realizar cotações de voos e hotéis. Inclui geocodificação autônoma de coordenadas e verificações robustas de datas.
- **Manutenção de Testes:** Suítes de testes unitários expandidas para cobrir as ferramentas da Duffel com 19/19 testes executando e passando de forma isolada e limpa.
