...
- **Mecanismo de Fallback Automático:** Adicionado um bloco de tratamento de exceção em `agentNode` no [src/agent.ts](file:///Users/leandropradopires/Projetos/mini_projeto/src/agent.ts). Se o modelo principal (ex: Groq) falhar devido a erros de limite de cota/tokens (429/413) da conta free, o fluxo aciona um fallback automático e transparente usando o OpenRouter com o modelo `llama-3.3-70b-instruct:free`, impedindo travamento de execução e mantendo o agente operacional de forma ininterrupta.
