...
Ao sair do **Nó do Agente**, a transição é avaliada por uma _Conditional Edge_ (Router):
...
- A inicialização do sistema valida a presença das variáveis de ambiente `GEMINI_API_KEY` e `GECKO_API_KEY`. Se ausentes, o sistema falha imediatamente (_Fail-Fast_) com uma mensagem explicativa.
...
- A sessão conversacional é indexada por um identificador exclusivo (`thread_id`), permitindo ao usuário fazer consultas refinadas consecutivas (ex: _"Mude o destino para Salvador"_ ou _"Ordene os hotéis pelo mais barato"_), mantendo o histórico.
